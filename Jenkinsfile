pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("iamrahulus/train-schedule")
                    app.inside {
            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'webserver', usernameVariable: 'USERNAME', passwordVariable: 'USERPASS')]) {
                    script {
                        ...
                        sh "sshpass -p '$USERPASS' -v ssh -o StrictHostKeyChecking=no $USERNAME@$Prod_IP \"docker run --restart always --name train-schedule -p 8080:8080 -d iamrahulus/train-schedule:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
